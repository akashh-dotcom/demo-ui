...
1. Invalid chapter content model - wraps disallowed content elements in sect1
...
This includes: para, figure, table, lists, programlisting, blockquote, admonitions (note, warning, etc.),
examples, equations, sidebars, procedures, abstracts, epigraphs, indexterms, and ANY other content.
"""
...
def wrap_chapter_content_in_sect1(chapter: etree._Element) -> int:
    ...
    According to ritthier2.mod DTD, chapters can only contain:
    - beginpage?, chapterinfo?, title, subtitle?, titleabbrev?, tocchap?, (toc|lot|index|glossary|bibliography|sect1)*

    GENERIC APPROACH: This function uses a whitelist of allowed elements.
    ALL other elements that appear as direct children of chapter are wrapped in sect1.

    This includes: para, figure, table, lists, programlisting, blockquote, note,
    warning, caution, important, tip, example, equation, sidebar, procedure,
    abstract, epigraph, indexterm, and ANY other content elements.
    ...
    # These are the ONLY elements that should appear directly under <chapter>
